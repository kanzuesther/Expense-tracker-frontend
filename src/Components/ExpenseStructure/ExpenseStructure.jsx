            ...
            .then((response) => {
                ...
                response.data.forEach((e) => {
                    if (e.type === 'expense') {
                        ...
                        if (Object.keys(chartData).includes(category.name)) {
                            chartData[category.name].total += e.amount;
                        } else {
                ...
                data.datasets=[
                    {
                        data: listOfCategories.map((category) => chartData[category].total),
                        backgroundColor: listOfCategories.map((category) => chartData[category].color),
                    }
                ]
    ...
    return (
        <div href="#" class="block p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
            ...
            <div className="flex flex-col mt-2">
                ...
                <div className="flex flex-row justify-between">
                    <h2 className="text-2xl font-bold"> {total > 0 ? '+ ' : '- '} FCFA {total}</h2>
            ...
            <div className='pt-2'>
                {data?.labels?.length && data?.datasets?.length && <DonutChart data={data} />}
            </div>
        </div>
    )
}
